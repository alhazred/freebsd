...
hc_prop_set(tnode_t *node, nvlist_t *auth)
{
	...
	*/
	...
	kenv(KENV_GET, "smbios.system.serial", serial, sizeof(serial));
	(void) topo_prop_set_string(node, FM_FMRI_AUTHORITY,
	    FM_FMRI_AUTH_CHASSIS, TOPO_PROP_IMMUTABLE, serial,
	    &err);
	...
	isa[0] = '\0';
	size_t isalen = sizeof(isa);
	sysctlbyname("hw.machine", isa, &isalen, NULL, -1);
	(void) uname(&uts);
