...
/*
 * Lookup string data named pname and add that
 * as property named pname in the given property group pgname on the indicated
 * topo node.
 */
int
add_strprop(topo_mod_t *mod, tnode_t *node,
...
/*
 * Lookup an int32 item named pname and add that
 * as property named pname in the given property group pgname on the indicated
 * topo node.  Fill pvalp with the property value.
 */
int
add_longprop(topo_mod_t *mod, tnode_t *node,
    ...
    const char *pgname, int32_t *pvalap, ...)
{
	...
	while ((pname = va_arg(ap, const char *)) != NULL) {
		if (add_longprop(mod, node, pgname, pname,
		    pvalap) != 0)
